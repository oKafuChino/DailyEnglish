...
def test_packaged_word_library_has_expected_distribution() -> None:
    ...
    assert counts == {"B1": 3600, "B2": 4800, "C1": 3600}
    assert len({word.text_en.casefold() for word in words}) == 12_000
    ...
    assert not any(
        marker in word.translation_zh for word in words for marker in BLOCKED_TRANSLATION_MARKERS
    )
